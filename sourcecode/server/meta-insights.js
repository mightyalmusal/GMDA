// server/meta-insights.js
// Local API proxy for Meta Graph API.
...
const MAPPING_OPTIONS_FILE = path.join(CACHE_DIR, "meta-selection-lists.json");
...
function normalizeMappingOptions(input) {
  const src = input && typeof input === "object" ? input : {};
  return {
    divisions: sanitizeOptionList(src.divisions, DEFAULT_MAPPING_OPTIONS.divisions),
    lobs: sanitizeOptionList(src.lobs, DEFAULT_MAPPING_OPTIONS.lobs),
    segments: sanitizeOptionList(src.segments, DEFAULT_MAPPING_OPTIONS.segments),
    objectives: sanitizeOptionList(src.objectives, DEFAULT_MAPPING_OPTIONS.objectives),
  };
}

function parseMappingOptionsFromIni(parsed) {
  const divisions = parseOptionList(parsed.mapping_divisions, DEFAULT_MAPPING_OPTIONS.divisions);
  const lobs = parseOptionList(parsed.mapping_lobs, DEFAULT_MAPPING_OPTIONS.lobs);
  const segments = parseOptionList(parsed.mapping_segments, DEFAULT_MAPPING_OPTIONS.segments);
  const objectives = parseOptionList(parsed.mapping_objectives, DEFAULT_MAPPING_OPTIONS.objectives);
  const hasLegacyKeys = ["mapping_divisions", "mapping_lobs", "mapping_segments", "mapping_objectives"].some(k => parsed[k] != null);
  return {
    hasLegacyKeys,
    mappingOptions: normalizeMappingOptions({ divisions, lobs, segments, objectives }),
  };
}

async function loadMappingOptionsFile() {
  try {
    const raw = await fs.readFile(MAPPING_OPTIONS_FILE, "utf8");
    const parsed = JSON.parse(raw);
    return normalizeMappingOptions(parsed?.mappingOptions || parsed || {});
  } catch (err) {
    if (err.code === "ENOENT") {
      // One-time migration path for old settings.ini mapping_* keys.
      try {
        const rawIni = await fs.readFile(SETTINGS_FILE, "utf8");
        const parsedIni = parseIni(rawIni);
        const legacy = parseMappingOptionsFromIni(parsedIni);
        if (legacy.hasLegacyKeys) {
          await saveMappingOptionsFile(legacy.mappingOptions);
          return legacy.mappingOptions;
        }
      } catch (iniErr) {
        if (iniErr.code !== "ENOENT") throw iniErr;
      }
      return normalizeMappingOptions(DEFAULT_MAPPING_OPTIONS);
    }
    throw err;
  }
}

async function saveMappingOptionsFile(mappingOptions) {
  const normalized = normalizeMappingOptions(mappingOptions);
  await fs.mkdir(CACHE_DIR, { recursive: true });
  await fs.writeFile(MAPPING_OPTIONS_FILE, JSON.stringify({ mappingOptions: normalized, updatedAt: new Date().toISOString() }), "utf8");
  return normalized;
}
...
exports.handler = async function (event) {
  ...
  if (action === "load_settings") {
    try {
      ...
      const mappingOptions = await loadMappingOptionsFile();
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ settings: { ...settings, mappingOptions } }),
      };
    } catch (err) {
      return { statusCode: 500, headers, body: JSON.stringify({ error: err.message }) };
    }
  ...
  if (action === "save_settings") {
    try {
      ...
      const mappingOptions = await saveMappingOptionsFile(body.mappingOptions || DEFAULT_MAPPING_OPTIONS);
      const settings = await loadSettingsIni();
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ ok: true, settings: { ...settings, mappingOptions } }),
      };
    } catch (err) {
      return { statusCode: 500, headers, body: JSON.stringify({ error: err.message }) };
    }
