...
const ACCESS_UNLOCK_KEY="easypc_config_unlocked";
const ACCESS_PASSWORD="marketingmafiasyndicate";
const LOCAL_ONLY=(import.meta.env.VITE_LOCAL_ONLY||"false").toLowerCase()==="true";
const LOCAL_CACHE_KEY="easypc_local_cache_rows";
...
function loadConfigUnlocked(){try{return localStorage.getItem(ACCESS_UNLOCK_KEY)==="1";}catch{return false;}}
function saveConfigUnlocked(v){try{localStorage.setItem(ACCESS_UNLOCK_KEY,v?"1":"0");}catch{}}
...
const DEFAULT_SETTINGS={accessToken:"",businessAccountId:"",appId:"",appSecret:"",mappingOptions:DEFAULT_MAPPING_OPTIONS,defaultMonth:"2026 MARCH",budgets:{"2026 JANUARY":1590000,"2026 FEBRUARY":1795298,"2026 MARCH":1897079},targets:{"2026 JANUARY":40000,"2026 FEBRUARY":43000,"2026 MARCH":46000},desktopBudgets:{},desktopTargets:{},ltsBudgets:{},ltsTargets:{},lsaBudgets:{},lsaTargets:{}};
...
function parseCSV(text){
  ...
  const fi=(...keys)=>{for(const k of keys){const i=headers.findIndex(h=>h.includes(k));if(i>=0)return i;}return -1;};
  const iName=fi("ad set name","adset name","adset_name","name"),iId=fi("ad set id","adset id","adset_id","id"),iDiv=fi("division","div"),iLob=fi("line of business","lob"),iSeg=fi("segment","seg"),iObj=fi("objective","obj");
  return lines.slice(1).map(line=>{
    const cols=line.split(",").map(c=>c.trim().replace(/^"|"$/g,""));
    const adset=iName>=0?cols[iName]:"",adsetId=iId>=0?cols[iId]:"";
...
// Data
function DataView({data,fetchMeta,syncStatus,onFacebookLogin,loginBusy,appId,tokenStatus,localOnly=false}){
  ...
  const pageRows=useMemo(()=>rows.slice(pageStart,pageStart+rowsPerPage),[rows,pageStart,rowsPerPage]);
  useEffect(()=>{setTablePage(1);},[rows.length,rowsPerPage]);
  const lastDate=rows.length?rows[0].day:null;
  const firstDate=rows.length?rows[rows.length-1].day:null;
  const fmtTs=ts=>{if(!ts)return"—";try{return new Date(ts).toLocaleString("en-PH",{month:"short",day:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit"});}catch{return ts;}};
  const etaText=useMemo(()=>{
    if(!syncStatus?.inProgress||!syncStatus?.startedAt)return"Estimating…";
    const total=Number(syncStatus.totalAccounts||0),done=Number(syncStatus.completedAccounts||0);
    if(!total||!done)return"Estimating…";
    const elapsed=Math.max(1,Math.floor((Date.now()-new Date(syncStatus.startedAt).getTime())/1000));
    const per=elapsed/done;
    const remain=Math.max(0,Math.round(per*(total-done)));
    const m=Math.floor(remain/60),s=remain%60;
    return `${m}m ${String(s).padStart(2,"0")}s`;
  },[syncStatus]);
  const statusItems=[
    {label:"Total rows fetched",value:rows.length.toLocaleString()},
    {label:"Latest date in data",value:lastDate||"—"},
    {label:"Last fetched at",value:fetchMeta?.fetchedAt?fmtTs(fetchMeta.fetchedAt):"Not recorded"},
    {label:"Date range",value:fetchMeta?.since&&fetchMeta?.until?`${fetchMeta.since} → ${fetchMeta.until}`:(firstDate&&lastDate?`${firstDate} → ${lastDate}`:"—")},
    {label:"Accounts queried",value:fetchMeta?.accountsQueried!=null?fetchMeta.accountsQueried:(fetchMeta?.discoveredAccounts?.length??"—")},
    {label:"Accounts succeeded",value:fetchMeta?.accountsSucceeded!=null?fetchMeta.accountsSucceeded:"—"},
  ];
  return(<div>
    {!localOnly&&<div className="card" style={{marginBottom:14}}>
    ...
    {syncStatus?.inProgress&&(
      <div className="card" style={{marginBottom:14,borderColor:"#12304D",background:"#081420"}}>
        <div className="card-hdr"><div className="card-ttl" style={{color:"#7DD3FC"}}>Meta API Sync Progress</div></div>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#93C5FD",marginBottom:8}}>
          ...
          <span style={{fontFamily:"'DM Mono',monospace"}}>ETA: {etaText}</span>
        </div>
        <div style={{height:8,background:"#0F2236",borderRadius:999,overflow:"hidden",border:"1px solid #1E3A5F"}}>
          <div style={{height:"100%",width:`${Math.min(100,Math.round(((syncStatus.completedAccounts||0)/Math.max(1,(syncStatus.totalAccounts||1)))*100))}%`,background:"linear-gradient(90deg,#0EA5E9,#60A5FA)",transition:"width .3s"}}/>
        </div>
        <div style={{marginTop:8,fontSize:12,color:"#60A5FA"}}>
          Account {syncStatus.currentAccountIndex||0}/{syncStatus.totalAccounts||0}: <span style={{fontFamily:"'DM Mono',monospace"}}>{syncStatus.currentAccountId||"—"}</span>
        </div>
      </div>
    )}
    <div className="card" style={{marginBottom:14}}>
      <div className="card-hdr"><div className="card-ttl">Sync Status</div></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:"10px 18px",padding:"4px 0 2px"}}>
        {statusItems.map(it=>(
          <div key={it.label} style={{background:"#F8FAFC",borderRadius:7,padding:"8px 12px",border:"1px solid #E2E8F0"}}>
            <div style={{fontSize:10,color:"#64748B",marginBottom:2,textTransform:"uppercase",letterSpacing:"0.04em"}}>{it.label}</div>
            <div style={{fontSize:13,fontWeight:600,color:"#1E293B",fontFamily:"'DM Mono',monospace"}}>{it.value}</div>
          </div>
        ))}
      </div>
      {fetchMeta?.errors?.length>0&&(
        <div style={{marginTop:10,background:"#FFF5F5",border:"1px solid #FCA5A5",borderRadius:6,padding:"8px 12px"}}>
          <div style={{fontSize:11,fontWeight:600,color:"#DC2626",marginBottom:4}}>Fetch errors ({fetchMeta.errors.length})</div>
          {fetchMeta.errors.map((e,i)=>(
            <div key={i} style={{fontSize:11,color:"#7F1D1D",marginBottom:2}}><span style={{fontFamily:"'DM Mono',monospace"}}>{e.accountId}</span>: {e.error}</div>
          ))}
        </div>
      )}
    </div>
    <div className="card"><div className="card-hdr"><div className="card-ttl">Fetched Meta Data ({rows.length.toLocaleString()} rows)</div></div>
      {rows.length>0&&(
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,gap:10,flexWrap:"wrap"}}>
          <div style={{fontSize:11.5,color:"#4B5563"}}>Showing {pageStart+1}–{Math.min(pageStart+rowsPerPage,rows.length)} of {rows.length.toLocaleString()}</div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <label style={{margin:0,fontSize:11.5,color:"#4B5563"}}>Rows/page</label>
            <select value={rowsPerPage} onChange={e=>setRowsPerPage(Number(e.target.value)||200)} style={{width:"auto",padding:"5px 8px",fontSize:12}}>
              {[100,200,500].map(n=><option key={n} value={n}>{n}</option>)}
            </select>
            <button className="btn" onClick={()=>setTablePage(p=>Math.max(1,p-1))} disabled={safePage<=1}>Prev</button>
            <div style={{fontSize:12,color:"#334155",minWidth:70,textAlign:"center"}}>{safePage} / {totalPages}</div>
            <button className="btn" onClick={()=>setTablePage(p=>Math.min(totalPages,p+1))} disabled={safePage>=totalPages}>Next</button>
          </div>
        </div>
      )}
      <div className="tw"><table>
        <thead><tr><th>Campaign name</th><th>Ad set name</th><th>Day</th><th>Account name</th><th>Currency</th><th>Amount spent (PHP)</th><th>Messaging conversations started</th><th>Post engagements</th><th>Reach</th><th>Impressions</th><th>Clicks (all)</th><th>Link clicks</th><th>Adds to cart</th><th>Adds to cart conversion value</th><th>Purchases conversion value</th><th>CTR (link click-through rate)</th><th>CTR (all)</th><th>Frequency</th></tr></thead>
        <tbody>{pageRows.map((r,i)=>(<tr key={`${r.account_id||""}-${r.adset_name||""}-${r.day||""}-${pageStart+i}`}><td className="td-p">{r.campaign_name||"-"}</td><td>{r.adset_name||"-"}</td><td>{r.day||"-"}</td><td>{r.account_name||"-"}</td><td>{r.currency||"PHP"}</td><td>{fmt(r.spend)}</td><td>{fmtN(r.inquiries)}</td><td>{fmtN(r.post_engagement)}</td><td>{fmtN(r.reach)}</td><td>{fmtN(r.impressions)}</td><td>{fmtN(r.clicks)}</td><td>{fmtN(r.link_clicks)}</td><td>{fmtN(r.add_to_cart)}</td><td>{fmt(r.add_to_cart_conversion_value)}</td><td>{fmt(r.purchases_conversion_value)}</td><td>{fmtP(r.ctr_link)}</td><td>{fmtP(r.ctr_all)}</td><td>{Number(r.frequency||0).toFixed(2)}</td></tr>))}</tbody>
      </table></div>
      {!rows.length&&<div className="empty" style={{padding:"16px 0"}}>No rows in source data yet.</div>}
    </div>
  </div>);
}
...
// Settings
function Settings({settings,setSettings,identifiers,toast,persistServerSettings}){
  const [loc,setLoc]=useState({...settings});
  const [newOpt,setNewOpt]=useState({divisions:"",lobs:"",segments:"",objectives:""});
  const listOf=(k)=>loc.mappingOptions?.[k]||DEFAULT_MAPPING_OPTIONS[k]||[];
  const addOption=(k)=>{
    const value=String(newOpt[k]||"").trim();
    if(!value)return;
    if(listOf(k).some(v=>String(v).toLowerCase()===value.toLowerCase()))return;
    setLoc(p=>({...p,mappingOptions:{...(p.mappingOptions||DEFAULT_MAPPING_OPTIONS),[k]:[...listOf(k),value]}}));
    setNewOpt(p=>({...p,[k]:""}));
  };
  ...
  const save=async()=>{
    ...
    try{
      ...
      toast(`Saved locally, but failed to write server settings files: ${e.message||"Unknown error"}`,true);
    }
  };
      ...
      {[
        ...
        <div key={key} style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:7,marginBottom:7}}>
            <input value={newOpt[key]} placeholder={`Add ${label} option`} onChange={e=>setNewOpt(p=>({...p,[key]:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addOption(key)} />
            <button className="btn btn-p" onClick={()=>addOption(key)} style={{whiteSpace:"nowrap"}}>Add</button>
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {listOf(key).map(v=><button key={v} className="btn" onClick={()=>removeOption(key,v)} title="Remove option" style={{padding:"3px 8px",fontSize:11}}>{v} ×</button>)}
          </div>
        </div>
      ))}
      <div style={{fontSize:11,color:"#64748B"}}>These lists are used by the Mapping page dropdown selections and saved to data/meta-selection-lists.json.</div>
    </div>
...
function BudgetTargets({settings,setSettings,identifiers,toast,persistBudgetTargets}){
  const [loc,setLoc]=useState({...settings});
  const [activeTab,setActiveTab]=useState("overall");
  const groupedMonths=useMemo(()=>({
    2025:MONTH_OPTIONS.filter(m=>m.startsWith("2025 ")),
    2026:MONTH_OPTIONS.filter(m=>m.startsWith("2026 ")),
  }),[]);

  const setMetric=(bucket,month,key,value)=>{
    setLoc(prev=>({
      ...prev,
      [bucket]:{
        ...(prev[bucket]||{}),
  ...
  const desktopTargetLabel=(seg)=>{
    if(seg==="Desktop Engagement")return "Desktop Engagement — Target Engagement";
    if(seg==="Expert-ease")return "Expert-ease — Target Engagement";
    return `${seg} — Target Inquiries`;
  };

  const ltsBudgetLabel=(seg)=>{
    if(seg==="Productivity")return "Productivity Inquiry — Budget (₱)";
    if(seg==="Gaming")return "Gaming — Inquiry Budget (₱)";
    return `${seg} — Budget (₱)`;
  };

  const desktopBudgetColumns=[
    {key:"Productivity",label:"Productivity — Budget (₱)"},
    {key:"Gaming",label:"Gaming — Budget (₱)"},
    {key:"High-end Gamer",label:"High-end Gamer — Budget (₱)"},
    {key:"Desktop Engagement",label:"Desktop Engagement — Budget (₱)"},
    {key:"Expert-ease",label:"Expert-ease — Budget (₱)"},
  ];
  const desktopTargetColumns=[
    {key:"Productivity",label:"Productivity — Target Inquiries"},
    {key:"Gaming",label:"Gaming — Target Inquiries"},
    {key:"High-end Gamer",label:"High-end Gamer — Target Inquiries"},
    {key:"Desktop Engagement",label:"Desktop Engagement — Engagement Inquiries"},
    {key:"Expert-ease",label:"Expert-ease — Target Inquiries"},
  ];
  ...
  const save=async()=>{
    ...
    saveLS({...loc,identifiers});
    try{
      await persistBudgetTargets({
        budgets:loc.budgets||{},
        targets:loc.targets||{},
        desktopBudgets:loc.desktopBudgets||{},
        desktopTargets:loc.desktopTargets||{},
        ltsBudgets:loc.ltsBudgets||{},
        ltsTargets:loc.ltsTargets||{},
        lsaBudgets:loc.lsaBudgets||{},
        lsaTargets:loc.lsaTargets||{},
      });
      toast("Budget and target settings saved ✓");
    }catch(e){
      toast(`Saved locally, but failed to write budget/targets file: ${e.message||"Unknown error"}`,true);
    }
  };

  return(<div>
    <div className="tabs" style={{marginBottom:14}}>
      <button className={`tab ${activeTab==="overall"?"active":""}`} onClick={()=>setActiveTab("overall")}>Overall</button>
      <button className={`tab ${activeTab==="desktop"?"active":""}`} onClick={()=>setActiveTab("desktop")}>Desktop</button>
      <button className={`tab ${activeTab==="lts"?"active":""}`} onClick={()=>setActiveTab("lts")}>LTS</button>
      <button className={`tab ${activeTab==="lsa"?"active":""}`} onClick={()=>setActiveTab("lsa")}>LSA</button>
    </div>

    {activeTab==="overall"&&(<div className="card">
      <div className="sec-ttl">Overall Monthly Budget & Targets</div>
      {[2025,2026].map(yr=>(<div key={yr} className="card" style={{marginBottom:12,padding:12}}>
        <div style={{fontSize:12,fontWeight:600,color:"#60A5FA",marginBottom:8}}>{yr}</div>
        {groupedMonths[yr].map(mo=>(<div className="fr fr2" key={mo} style={{marginBottom:8}}>
          <div><label style={{color:"#60A5FA"}}>{mo} — Budget (PHP)</label><input type="number" placeholder="e.g. 1590000" value={loc.budgets?.[mo]||""} onChange={e=>setLoc(p=>({...p,budgets:{...p.budgets,[mo]:+e.target.value}}))}/></div>
          <div><label>{mo} — Target Inquiries</label><input type="number" placeholder="e.g. 40000" value={loc.targets?.[mo]||""} onChange={e=>setLoc(p=>({...p,targets:{...p.targets,[mo]:+e.target.value}}))}/></div>
        </div>))}
      </div>))}
    </div>)}

    {activeTab==="desktop"&&(<div className="card">
      <div className="sec-ttl">Desktop Dashboard — Segment Budgets & Targets</div>
      {[2025,2026].map(yr=>(<div key={yr} className="card" style={{marginBottom:12,padding:12}}>
        <div style={{fontSize:13,fontWeight:600,color:"#60A5FA",marginBottom:8}}>{yr}</div>

        <div style={{fontSize:12,fontWeight:600,color:"#94A3B8",marginBottom:6}}>BUDGET</div>
        <div className="tw" style={{marginBottom:10}}><table>
          <thead><tr><th>Month</th>{desktopBudgetColumns.map(c=><th key={`bh-${yr}-${c.key}`}>{c.label}</th>)}</tr></thead>
          <tbody>{groupedMonths[yr].map(mo=>(<tr key={`br-${yr}-${mo}`}>
            <td className="td-p">{mo}</td>
            {desktopBudgetColumns.map(c=>(<td key={`bc-${yr}-${mo}-${c.key}`}><input type="number" placeholder="0" value={loc.desktopBudgets?.[mo]?.[c.key]||""} onChange={e=>setMetric("desktopBudgets",mo,c.key,e.target.value)} style={{minWidth:120}} /></td>))}
          </tr>))}</tbody>
        </table></div>

        <div style={{fontSize:12,fontWeight:600,color:"#94A3B8",marginBottom:6}}>TARGET</div>
        <div className="tw"><table>
          <thead><tr><th>Month</th>{desktopTargetColumns.map(c=><th key={`th-${yr}-${c.key}`}>{c.label}</th>)}</tr></thead>
          <tbody>{groupedMonths[yr].map(mo=>(<tr key={`tr-${yr}-${mo}`}>
            <td className="td-p">{mo}</td>
            {desktopTargetColumns.map(c=>(<td key={`tc-${yr}-${mo}-${c.key}`}><input type="number" placeholder="0" value={loc.desktopTargets?.[mo]?.[c.key]||""} onChange={e=>setMetric("desktopTargets",mo,c.key,e.target.value)} style={{minWidth:120}} /></td>))}
          </tr>))}</tbody>
        </table></div>
      </div>))}
    </div>)}

    {activeTab==="lts"&&(<div className="card">
      <div className="sec-ttl">LTS Dashboard — Segment Budgets & Targets</div>
      {[2025,2026].map(yr=>(<div key={yr} className="card" style={{marginBottom:12,padding:12}}>
        <div style={{fontSize:13,fontWeight:600,color:"#60A5FA",marginBottom:8}}>{yr}</div>

        <div style={{fontSize:12,fontWeight:600,color:"#94A3B8",marginBottom:6}}>BUDGET</div>
        <div className="tw" style={{marginBottom:10}}><table>
          <thead><tr><th>Month</th><th>Productivity — Budget (₱)</th><th>Gaming — Budget (₱)</th><th>Productivity Engagement — Budget (₱)</th><th>Gaming Engagement — Budget (₱)</th></tr></thead>
          <tbody>{groupedMonths[yr].map(mo=>(<tr key={`lts-b-${yr}-${mo}`}>
            <td className="td-p">{mo}</td>
            <td><input type="number" placeholder="0" value={loc.ltsBudgets?.[mo]?.["Productivity"]||""} onChange={e=>setMetric("ltsBudgets",mo,"Productivity",e.target.value)} style={{minWidth:120}} /></td>
            <td><input type="number" placeholder="0" value={loc.ltsBudgets?.[mo]?.["Gaming"]||""} onChange={e=>setMetric("ltsBudgets",mo,"Gaming",e.target.value)} style={{minWidth:120}} /></td>
            <td><input type="number" placeholder="0" value={loc.ltsBudgets?.[mo]?.["Productivity Engagement"]||""} onChange={e=>setMetric("ltsBudgets",mo,"Productivity Engagement",e.target.value)} style={{minWidth:120}} /></td>
            <td><input type="number" placeholder="0" value={loc.ltsBudgets?.[mo]?.["Gaming Engagement"]||""} onChange={e=>setMetric("ltsBudgets",mo,"Gaming Engagement",e.target.value)} style={{minWidth:120}} /></td>
          </tr>))}</tbody>
        </table></div>

        <div style={{fontSize:12,fontWeight:600,color:"#94A3B8",marginBottom:6}}>TARGET</div>
        <div className="tw"><table>
          <thead><tr><th>Month</th><th>Productivity — Target Inquiries</th><th>Gaming — Target Inquiries</th></tr></thead>
          <tbody>{groupedMonths[yr].map(mo=>(<tr key={`lts-t-${yr}-${mo}`}>
            <td className="td-p">{mo}</td>
            <td><input type="number" placeholder="0" value={loc.ltsTargets?.[mo]?.["Productivity"]||""} onChange={e=>setMetric("ltsTargets",mo,"Productivity",e.target.value)} style={{minWidth:120}} /></td>
            <td><input type="number" placeholder="0" value={loc.ltsTargets?.[mo]?.["Gaming"]||""} onChange={e=>setMetric("ltsTargets",mo,"Gaming",e.target.value)} style={{minWidth:120}} /></td>
          </tr>))}</tbody>
        </table></div>
      </div>))}
    </div>)}

    {activeTab==="lsa"&&(<div className="card">
      <div className="sec-ttl">LSA Dashboard — Segment Budgets</div>
      {[2025,2026].map(yr=>(<div key={yr} className="card" style={{marginBottom:12,padding:12}}>
        <div style={{fontSize:13,fontWeight:600,color:"#60A5FA",marginBottom:8}}>{yr}</div>
        <div style={{fontSize:12,fontWeight:600,color:"#94A3B8",marginBottom:6}}>BUDGET</div>
        <div className="tw"><table>
          <thead><tr><th>Store</th>{groupedMonths[yr].map(mo=><th key={`lsa-h-${yr}-${mo}`}>{mo}</th>)}</tr></thead>
          <tbody>{LSA_DASH_SEGMENTS.map(seg=>(<tr key={`lsa-r-${yr}-${seg}`}>
            <td className="td-p">{seg}</td>
            {groupedMonths[yr].map(mo=>(<td key={`lsa-c-${yr}-${seg}-${mo}`}><input type="number" placeholder="0" value={loc.lsaBudgets?.[mo]?.[seg]||""} onChange={e=>setMetric("lsaBudgets",mo,seg,e.target.value)} style={{minWidth:120}} /></td>))}
          </tr>))}</tbody>
        </table></div>
      </div>))}
    </div>)}

    <div style={{display:"flex",gap:9}}><button className="btn btn-p" onClick={save}>Save Budget and Target</button><button className="btn" onClick={()=>setLoc({...settings})}>Discard</button></div>
  </div>);
}

function Mapping({settings,setSettings,identifiers,setIdentifiers,toast,rawData,persistMappings}){
  const [loc,setLoc]=useState({...settings}),[ids,setIds]=useState([...identifiers]),[adsetIn,setAdsetIn]=useState(""),[drag,setDrag]=useState(false);
  const [tab,setTab]=useState("mapping");
  const [adsetSearch,setAdsetSearch]=useState("");
  const [adsetMappedFilter,setAdsetMappedFilter]=useState("all");
  ...
  const remId=i=>setIds(p=>p.filter((_,j)=>j!==i));
  const updId=(i,k,v)=>setIds(p=>p.map((r,j)=>j===i?{...r,[k]:v}:r));
  const handleCSV=text=>{const parsed=parseCSV(text);if(!parsed.length){toast("No valid rows found in CSV",true);return;}setIds(prev=>{const m=[...prev];parsed.forEach(p=>{const ei=m.findIndex(r=>(p.adsetId&&r.adsetId===p.adsetId)||(r.adset===p.adset));if(ei>=0)m[ei]={...m[ei],...p};else m.push(p);});return m;});toast(`Imported ${parsed.length} mappings ✓`);};
  const onFile=e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=ev=>handleCSV(ev.target.result);r.readAsText(f);e.target.value="";};
  const onDrop=e=>{e.preventDefault();setDrag(false);const f=e.dataTransfer.files?.[0];if(!f)return;const r=new FileReader();r.onload=ev=>handleCSV(ev.target.result);r.readAsText(f);};
  const save=async()=>{
    setSettings(loc);
    setIdentifiers(ids);
    saveLS({...loc,identifiers:ids});
    try{
      await persistMappings(ids);
      toast("Mappings saved ✓");
    }catch(e){
      toast(`Saved locally, but failed to write mappings file: ${e.message||"Unknown error"}`,true);
    }
  };
  const accountStats=useMemo(()=>{
    ...
    (rawData||[]).forEach(r=>{
      ...
      if(!id)return;
      if(!map[id])map[id]={accountId:id,accountName:r?.account_name||"(no account name)",rows:0,minDay:r?.day||null,maxDay:r?.day||null,adsets:new Set()};
      map[id].rows+=1;
      if(r?.adset_name)map[id].adsets.add(r.adset_name);
      if(r?.day){
        if(!map[id].minDay||r.day<map[id].minDay)map[id].minDay=r.day;
        if(!map[id].maxDay||r.day>map[id].maxDay)map[id].maxDay=r.day;
      }
    });
    return Object.values(map).map(v=>({...v,uniqueAdsets:v.adsets.size})).sort((a,b)=>b.rows-a.rows);
  },[rawData]);
  useEffect(()=>{
    const ids=accountStats.map(a=>a.accountId);
    setSelectedAccounts(prev=>{
      const retained=prev.filter(id=>ids.includes(id));
      return retained.length?retained:ids;
    });
  },[accountStats]);
  const filteredRows=useMemo(()=>{
    if(!selectedAccounts.length)return [];
    const set=new Set(selectedAccounts);
    return (rawData||[]).filter(r=>set.has(String(r?.account_id||"")));
  },[rawData,selectedAccounts]);
  const uniqueAdsets=useMemo(()=>{
    const set=new Set(filteredRows.map(r=>String(r?.adset_name||"").trim()).filter(Boolean));
    return [...set].sort((a,b)=>a.localeCompare(b));
  },[filteredRows]);
  const mappedNameSet=useMemo(()=>new Set(ids.map(i=>String(i.adset||"").trim().toLowerCase()).filter(Boolean)),[ids]);
  const visibleAdsets=useMemo(()=>{
    const q=adsetSearch.trim().toLowerCase();
    return uniqueAdsets.filter(name=>{
      const nm=name.toLowerCase();
      if(q&&!nm.includes(q))return false;
      const mapped=mappedNameSet.has(nm);
      if(adsetMappedFilter==="mapped"&&!mapped)return false;
      if(adsetMappedFilter==="unmapped"&&mapped)return false;
      return true;
    });
  ...
  const addFromData=(name)=>{
    ...
    if(!value)return;
    const exists=ids.some(i=>String(i.adset||"").trim().toLowerCase()===value.toLowerCase());
    if(exists)return;
    setIds(p=>[...p,{adset:value,adsetId:"",division:"",lob:"",segment:"",objective:""}]);
  };
  return(<div>
    <div className="tabs" style={{marginBottom:14}}>
      <button className={`tab ${tab==="mapping"?"active":""}`} onClick={()=>setTab("mapping")}>Mapping Editor</button>
      <button className={`tab ${tab==="adsets"?"active":""}`} onClick={()=>setTab("adsets")}>Ad Set Names</button>
    </div>
    {tab==="mapping"&&(<>
    <div className="card">
      <div className="sec-ttl">Ad Set Identifier Mapping</div>
      <p style={{fontSize:11.5,color:"#374151",marginBottom:11}}>Upload a CSV or add entries manually. CSV columns: <code style={{fontFamily:"'DM Mono',monospace",color:"#60A5FA",fontSize:10.5}}>Ad Set Name, Ad Set ID, Division, LOB, Segment, Objective</code>. Ad Set ID resolves duplicates across accounts.</p>
      <div className={`drop-zone ${drag?"drag":""}`} onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)} onDrop={onDrop} onClick={()=>fRef.current?.click()}>
        <svg width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="#374151" strokeWidth="1.5" style={{margin:"0 auto"}}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
        <p>Drop .csv here, or click to browse</p>
        <small>Supports CSV exported from Meta Ads Manager or your own mapping sheet</small>
    ...
    {tab==="adsets"&&(<div className="card">
      <div className="sec-ttl">Unique Ad Set Names From Data</div>
      <div style={{fontSize:12,color:"#64748B",marginBottom:10}}>Filtered Data rows: {filteredRows.length.toLocaleString()} · {uniqueAdsets.length.toLocaleString()} unique ad set names.</div>
      <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}>
        <input value={adsetSearch} onChange={e=>setAdsetSearch(e.target.value)} placeholder="Search ad set name..." style={{maxWidth:360}} />
        <select value={adsetMappedFilter} onChange={e=>setAdsetMappedFilter(e.target.value)} style={{width:"auto",minWidth:180}}>
          <option value="all">All statuses</option>
          <option value="mapped">Mapped only</option>
          <option value="unmapped">Not yet mapped</option>
        </select>
        {adsetSearch&&<button className="btn" onClick={()=>setAdsetSearch("")}>Clear</button>}
      </div>
      <div className="card" style={{marginBottom:12,padding:12}}>
        <div className="card-hdr" style={{marginBottom:8}}><div className="card-ttl">Data Audit</div></div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
          <button className="btn" onClick={()=>setSelectedAccounts(accountStats.map(a=>a.accountId))}>Select All</button>
          <button className="btn" onClick={()=>setSelectedAccounts([])}>Clear All</button>
        </div>
        <div className="tw"><table>
          <thead><tr><th>Use</th><th>Account ID</th><th>Account Name</th><th>Rows</th><th>Unique Ad Sets</th><th>Min Day</th><th>Max Day</th></tr></thead>
          <tbody>{accountStats.map(a=>{const checked=selectedAccounts.includes(a.accountId);return(<tr key={a.accountId}><td><input type="checkbox" checked={checked} onChange={e=>setSelectedAccounts(prev=>e.target.checked?[...new Set([...prev,a.accountId])]:prev.filter(id=>id!==a.accountId))} /></td><td>{a.accountId}</td><td className="td-p">{a.accountName}</td><td>{fmtN(a.rows)}</td><td>{fmtN(a.uniqueAdsets)}</td><td>{a.minDay||"-"}</td><td>{a.maxDay||"-"}</td></tr>);})}</tbody>
        </table></div>
      </div>
      {!uniqueAdsets.length&&<div className="empty" style={{padding:"16px 0"}}>No ad set names found in Data yet. Fetch Data first.</div>}
      {uniqueAdsets.length>0&&visibleAdsets.length===0&&<div className="empty" style={{padding:"16px 0"}}>No ad set names match your search.</div>}
      {visibleAdsets.length>0&&(<>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:8,flexWrap:"wrap"}}>
        <div style={{fontSize:12,color:"#64748B"}}>Showing {adsetStart+1}–{Math.min(adsetStart+adsetRowsPerPage,visibleAdsets.length)} of {visibleAdsets.length.toLocaleString()}</div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <label style={{margin:0,fontSize:11.5,color:"#64748B"}}>Rows/page</label>
          <select value={adsetRowsPerPage} onChange={e=>setAdsetRowsPerPage(Number(e.target.value)||20)} style={{width:"auto",padding:"5px 8px",fontSize:12}}>
            {[20,50,100].map(n=><option key={n} value={n}>{n}</option>)}
          </select>
          <button className="btn" onClick={()=>setAdsetPage(p=>Math.max(1,p-1))} disabled={adsetSafePage<=1}>Prev</button>
          <div style={{fontSize:12,color:"#334155",minWidth:70,textAlign:"center"}}>{adsetSafePage} / {adsetTotalPages}</div>
          <button className="btn" onClick={()=>setAdsetPage(p=>Math.min(adsetTotalPages,p+1))} disabled={adsetSafePage>=adsetTotalPages}>Next</button>
        </div>
      </div>
      <div className="tw"><table>
        <thead><tr><th>Ad Set Name</th><th>Action</th></tr></thead>
        <tbody>{pagedAdsets.map(name=>{const mapped=mappedNameSet.has(name.toLowerCase());return(<tr key={name}><td className="td-p">{name}</td><td>{mapped?<span className="pill pl-grn">Mapped</span>:<button className="btn btn-p" onClick={()=>addFromData(name)} style={{padding:"4px 10px",fontSize:12}}>Add to Mapping</button>}</td></tr>);})}</tbody>
      </table></div>
      </>)}
      <div style={{display:"flex",gap:9,marginTop:12}}><button className="btn btn-p" onClick={save}>Save Mapping</button><button className="btn" onClick={()=>setTab("mapping")}>Back to Editor</button></div>
    </div>)}
  </div>);
}
...
const NAV=[
  ...
  {id:"lts",       label:"LTS Dashboard",     g:"dash",icon:"M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4"},
  {id:"lsa",       label:"LSA Dashboard",     g:"dash",icon:"M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z"},
  {id:"trends",    label:"Trends & Graphs",   g:"dash",icon:"M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"},
  {id:"settings",  label:"Settings",          g:"cfg", icon:"M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z"},
  {id:"budget",    label:"Budget and Target", g:"cfg", icon:"M3 10h18M6 6h12M6 14h12M9 18h6"},
...
export default function App(){
  ...
  const [identifiers,setIdentifiers]=useState(()=>saved?.identifiers||[]);
  ...
  const signOutMicrosoft=useCallback(async()=>{
    ...
    setApiAuthToken("");
    setOrgAccount(null);
    setOrgAuthBusy(false);
    showToast("Signed out from Microsoft account");
  },[msalClient,orgAccount]);

  const unlockProtectedPages=(e)=>{
    e.preventDefault();
    if(passwordInput===ACCESS_PASSWORD){
      setConfigUnlocked(true);
      setPasswordInput("");
      setPasswordError("");
      showToast("Access granted for configuration pages ✓");
      return;
    }
  ...
  useEffect(()=>{
    saveConfigUnlocked(configUnlocked);
  },[configUnlocked]);
  ...
  useEffect(()=>{
    ...
    const boot=async()=>{
      if(LOCAL_ONLY){
        setOrgAuthReady(true);
        setOrgAccount({username:"local@localhost"});
        setApiAuthToken("");
        return;
      }
  ...
  useEffect(()=>{
    ...
    try{
      ...
      setRawData(rows);
      setFetchMeta(localMetaFromRows(rows));
      setSyncStatus(null);
      setApiErrors([]);
    }catch{}
  },[]);

  const importLocalCacheFile=useCallback(async(file)=>{
    if(!file)return;
    setLoading(true);
    try{
      ...
      setSyncStatus(null);
      setApiErrors([]);
      try{localStorage.setItem(LOCAL_CACHE_KEY,JSON.stringify({rows}));}catch{}
      showToast(`Loaded local cache (${rows.length.toLocaleString()} rows) ✓`);
    }catch(e){
      showToast(`Failed to load local JSON: ${e.message||"Invalid file"}`,true);
  ...
  const loadServerMappings=useCallback(async()=>{
    try{
      const r=await loadMappingsFromApi();
      const serverIds=Array.isArray(r?.identifiers)?r.identifiers:[];
      if(serverIds.length){
        setIdentifiers(serverIds);
      }else{
        const local=loadLS()?.identifiers;
        if(Array.isArray(local)&&local.length){
          await saveMappingsToApi(local);
        }
      }
    }catch{/* optional file; ignore on startup */}
  },[]);

  const loadServerBudgetTargets=useCallback(async()=>{
    try{
      const r=await loadBudgetTargetsFromApi();
      const bt=r?.budgetTargets||{};
      const hasAny=["budgets","targets","desktopBudgets","desktopTargets","ltsBudgets","ltsTargets","lsaBudgets","lsaTargets"].some(k=>bt&&typeof bt[k]==="object"&&bt[k]&&Object.keys(bt[k]).length>0);
      if(hasAny){
        setSettings(prev=>({
          ...prev,
          budgets:bt.budgets||prev.budgets||{},
          targets:bt.targets||prev.targets||{},
          desktopBudgets:bt.desktopBudgets||prev.desktopBudgets||{},
          desktopTargets:bt.desktopTargets||prev.desktopTargets||{},
          ltsBudgets:bt.ltsBudgets||prev.ltsBudgets||{},
          ltsTargets:bt.ltsTargets||prev.ltsTargets||{},
          lsaBudgets:bt.lsaBudgets||prev.lsaBudgets||{},
          lsaTargets:bt.lsaTargets||prev.lsaTargets||{},
        }));
      }
    }catch{/* optional file; ignore on startup */}
  },[]);

  const persistServerSettings=useCallback(async(accessToken,businessAccountId,appId,appSecret,mappingOptions)=>{
    await saveSettingsToApi(accessToken,businessAccountId,appId,appSecret,mappingOptions);
  },[]);

  const persistMappings=useCallback(async(identifiersToSave)=>{
    await saveMappingsToApi(identifiersToSave);
  },[]);
  ...
  const clearCache=useCallback(async()=>{
    ...
    if(step2!=="CLEAR CACHE"){
      showToast("Clear cache cancelled: confirmation text did not match.",true);
      return;
    }
    setLoading(true);
    try{
      const r=await clearCacheFromApi();
      setRawData(r.data||[]);
      setFetchMeta(r.meta||null);
      setSyncStatus(null);
      setDiscoveredAccounts([]);
      setBusinessName(null);
      showToast("Cache cleared. Next fetch will start from 2025-01-01 \u2713");
    }catch(e){
  ...
  const fetchData=useCallback(async()=>{
    if(LOCAL_ONLY){
      showToast("Local mode is enabled. Click Load Local Cache JSON.",true);
      return;
    }
    ...
    try{
      ...
      if(r.meta){
        ...
        const hasTokenError=Boolean(effectiveErrors.some(e=>{
          const msg=String(e?.error||"").toLowerCase();
          return msg.includes("code=190")||msg.includes("subcode=463")||msg.includes("access token")||msg.includes("oauthexception")||msg.includes("oauth");
        }));
        if(hasTokenError)setTokenStatus("invalid");
        else if(!effectiveErrors.length||(r.meta.accountsSucceeded||0)>0)setTokenStatus("valid");
        ...
        if(effectiveErrors.length){
          ...
          setApiErrors([]);
          showToast(`Fetched ${(r.data||[]).length.toLocaleString()} rows from Meta API \u2713`);
        }
      }else{
        setApiErrors([]);
        showToast(`Fetched ${(r.data||[]).length.toLocaleString()} rows from Meta API \u2713`);
  ...
  useEffect(()=>{
    ...
    loadServerSettings();
    loadServerMappings();
    loadServerBudgetTargets();
  },[orgAccount,loadCachedData,loadServerSettings,loadServerMappings,loadServerBudgetTargets]);
  ...
  return(<>
    ...
    <div className="app">
      <aside className={`sidebar ${navCollapsed?"collapsed":""}`}>
        ...
        <div className="sb-bot">
          <div className="sb-meta">
            ...
            <div style={{display:"flex",alignItems:"center",gap:5}}><span className="sdot" style={{background:COLORS.green}}/><span style={{color:"#60A5FA",fontWeight:500}}>Live Meta API</span></div>
            {(<>
            <div style={{display:"flex",alignItems:"center",gap:5}}>
              <span style={{width:6,height:6,borderRadius:"50%",background:tokenStatus==="valid"?"#22C55E":tokenStatus==="invalid"?"#EF4444":"#6B7280",display:"inline-block",flexShrink:0}}/>
              <span style={{color:tokenStatus==="valid"?"#86EFAC":tokenStatus==="invalid"?"#FCA5A5":"#6B7280"}}>{tokenStatus==="valid"?"Token valid":tokenStatus==="invalid"?"Token invalid":"Not verified"}</span>
            </div>
            {businessName&&<div style={{color:"#93C5FD",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",paddingLeft:11}} title={businessName}>{businessName}</div>}
            {discoveredAccounts.length>0&&<div style={{color:"#475569",paddingLeft:11}}>{discoveredAccounts.length} account(s)</div>}
            </>)}
          </div>
          ...
          <button className={`theme-toggle ${theme==="light"?"light":""}`} onClick={()=>setTheme(t=>t==="dark"?"light":"dark")} title={theme==="dark"?"Switch to light mode":"Switch to dark mode"}>
            <span className="theme-pill" aria-hidden="true">
              <span className="ico sun">☀</span>
              <span className="ico moon">☾</span>
      ...
      <div className="main">
        <div className="topbar">
          ...
          {page==="data"?(
            <div className="topbar-r">
              {LOCAL_ONLY?(
                <>
                  <button className="btn btn-p" onClick={()=>localCacheInputRef.current?.click()} disabled={loading||protectedPage}>Load Local Cache JSON</button>
                  <input
                    ref={localCacheInputRef}
                    type="file"
                    accept="application/json,.json"
                    style={{display:"none"}}
                    onChange={async e=>{const f=e.target.files?.[0];if(f)await importLocalCacheFile(f);e.target.value="";}}
                  />
                </>
              ):(
        ...
        {loading&&page==="data"&&<div><div className="fetch-bar"/><div style={{padding:"7px 22px",fontSize:12,color:"#60A5FA",background:"#0B1220",borderBottom:"1px solid #1F2937"}}>Fetching data from Meta API... this may take a while for large ranges.</div></div>}
        <div className="scroll-area">
          ...
          {loading&&page!=="data"?(<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,padding:60,color:"#2D3B50",fontSize:14}}><span className="spin"/>Fetching from Meta API…</div>):(<div key={page} className="page-switch">
            ...
            {page==="data"      &&<div className="guard-wrap"><div className={`guard-content ${protectedPage?"is-locked":""}`}><DataView data={rawData} fetchMeta={fetchMeta} syncStatus={syncStatus} onFacebookLogin={loginWithFacebook} loginBusy={loginBusy} appId={settings.appId} tokenStatus={tokenStatus} localOnly={LOCAL_ONLY}/></div>{protectedPage&&<div className="guard-overlay"><form className="guard-card" onSubmit={unlockProtectedPages}><div className="guard-title">Protected Page</div><div className="guard-sub">Enter password to access Settings, Budget and Target, Mapping, and Data.</div><label>Password</label><input type="password" value={passwordInput} onChange={e=>{setPasswordInput(e.target.value);if(passwordError)setPasswordError("");}} placeholder="Enter access password" autoFocus />{passwordError&&<div className="guard-err">{passwordError}</div>}<div style={{marginTop:10,display:"flex",justifyContent:"flex-end"}}><button className="btn btn-p" type="submit">Unlock</button></div></form></div>}</div>} 
          </div>)}
        </div>
      </div>
    </div>
