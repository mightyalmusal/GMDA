...
function localPublishApi() {
  const fnPath = resolve(process.cwd(), 'server/publish.js')
  const cjsRequire = createRequire(import.meta.url)
    ...
    configureServer(server) {
      server.middlewares.use('/api/publish', async (req, res) => {
        try {
          ...
          if (typeof handler !== 'function') {
            throw new Error('publish handler not found')
          }
...
export default defineConfig(({ mode }) => {
  // Load ALL env vars (not just VITE_ prefixed) so server middleware can read them via process.env
  const env = loadEnv(mode, process.cwd(), '')
  Object.assign(process.env, env)

  return {
    plugins: [react(), localMetaInsightsApi(), localPublishApi()],
  }
})
